...
<?php
//	3.6 文件的锁定机制	
echo "<h2>3.6 文件的锁定机制</h2>";	 
$nameMsg="text_data.txt";   //声明一个变量保存文件名，在这个文件中保存留言信息

if(file_exists($nameMsg)){
	readMsgFile();	
}
function readMsgFile(){
	global $nameMsg;
	$handler = fopen($nameMsg, "r");	
	flock($handler, LOCK_SH);
	$buffer = '';
	while(!feof($handler)){
		if(filesize($nameMsg) != 0){
			$buffer .= fread($handler, filesize($nameMsg));
		}else{
			return;
		}
	}	
	$arrExplode = explode("<|>", $buffer);
	foreach($arrExplode as $value){
		//把数组中的值赋给一些变量 
		list($a, $b, $c) = explode("||", $value);
		//if($username!="" && $title!="" && $message!="")
		if($a != "" && $b !="" && $c!=""){
			echo "userId:".$a.", 标题:".$b.",内容:".$c."<hr>";
		}
	}
